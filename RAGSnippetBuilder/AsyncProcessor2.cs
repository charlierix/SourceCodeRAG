...
{
    public class AsyncProcessor2<TInput, TOutput>
    {
        //public record ServiceFactory
        //{
        ...
        private readonly Func<Task<Func<TInput, Task<TOutput>>>> _serviceFactory;
        private readonly int _maxConcurrency;

        public AsyncProcessor2(Func<Task<Func<TInput, Task<TOutput>>>> serviceFactory, int maxConcurrency)
        {
        ...
        private void StartConsumers()
        {
            for (int i = 0; i < _maxConcurrency; i++)
            {
                Task.Run(async () =>
                {
                    var service = await _serviceFactory(); // Create a new instance of the service object for each consumer task
